    ...
    public partial class InventorBuildRibbon
    {
        ...
        private List<CommandItem> GetCommands()
        {
            return new List<CommandItem>
            {
                ...
                new CommandItem(Commands.RunMacro, "Path to the SOLIDWORKS macro (.swp). Type only the file name to run a macro stored next to this Excel file, or type a full path for a fixed location", ConstantStrings.ParentText, "Name of the procedure to run inside the macro. Leave blank to run the default procedure (Main)", applicationType: ApplicationTypeEnum.Solidworks),                new CommandItem(Commands.SetLevelOfDetail, "Name of the Level Detail", ConstantStrings.ParentText, "", "", "", "Activates the level detail and creates it if not does not exist", applicationType: ApplicationTypeEnum.Inventor),
                                new CommandItem(Commands.GetViewScale, "Name of the drawing view exactly as shown in SOLIDWORKS. Leave blank to read the active sheet scale", ConstantStrings.ParentText, "Application will set the view scale here (for example 0.5 means 1:2)", applicationType: ApplicationTypeEnum.Solidworks),
